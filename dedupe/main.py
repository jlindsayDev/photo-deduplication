...
def fetch_library_photos(library):
  photosdb = PhotosDB(library.path)
...
def main() -> int:
    ...
    applephotos, directories = fetch_libraries(args.paths, db_session)
    photos, videos, albums = fetch_library_photos(applephotos[0]) # TODO

    if args.dry_run:
        logging.info('[dry_run] skipping photo persistence')
        logging.info('[dry_run] skipping image encoding')
        logging.info('[dry_run] skipping deduplication check and persistence')
        return 0
